...
def dcf_from_fcf_list(fcf_list, discount_rate, terminal_growth, method="gordon"):
    """
    method: 'gordon' for Gordon growth, 'ev/fcf' for EV/FCF terminal
    """
    pv_years = []
    ...
    if method == "gordon":
        terminal = last_fcf * (1 + terminal_growth) / (discount_rate - terminal_growth)
    else:  # EV/FCF multiple
        ev_multiple = method if isinstance(method, (int,float)) else 20
        terminal = last_fcf * ev_multiple
...
if selected_display != "Select a stock...":
    ...
    shares_outstanding_safe = correct_shares_outstanding(
        safe_int(info.get("sharesOutstanding"), 0),
        safe_float(info.get("previousClose"), 0),
        safe_float(info.get("marketCap"), 0)
    )
    market_price_safe = safe_float(info.get("previousClose"), 0.0)
    market_cap_safe = safe_float(info.get("marketCap"), 0.0)
    long_name = info.get("longName", ticker_symbol)
    
    fcf_ttm = safe_get_cashflow_avg(ticker_yf, years=3) or 1_000_000_000
    bal = safe_get_balance_items(ticker_yf)
    cash_guess, debt_guess = safe_float(bal.get('cash')), safe_float(bal.get('totalDebt'))
    ...
    # --- User inputs ---
    shares_outstanding_input = st.number_input("Shares Outstanding:", min_value=0, value=int(shares_outstanding_safe), step=1)
    starting_fcf_input = st.number_input(
        "Starting FCF (TTM) USD:", min_value=0.0, value=fcf_ttm, step=1_000_000.0, format="%.0f"
    )
    
    # --- Growth sliders ---
    st.markdown("### 5-Year Growth Rate Assumptions")
    growth_cols = st.columns(5)
    user_growth_rates = [growth_cols[i].slider(f"Year {i+1} Growth %", 0.0, 100.0, 20.0, step=1.0) for i in range(5)]
    ...
    terminal_method = term_cols[1].selectbox("Method", ["Gordon Growth", "EV/FCF Multiple"])
    if terminal_method == "Gordon Growth":
        terminal_growth = term_cols[2].slider("Terminal Growth Rate %", 0.0, 10.0, 3.0, step=0.1)/100
        term_value_method = "gordon"
    else:
        ev_multiple = term_cols[2].slider("EV/FCF Multiple", 5, 40, 20)
        terminal_growth = 0.0
        term_value_method = ev_multiple
    
    # --- Discount rates ---
    st.markdown("### Discount Rate Scenarios")
    disc_cols = st.columns(3)
    disc_bull = disc_cols[0].slider("Bull Discount Rate %", 0.0, 50.0, 8.0, 0.1)/100
    disc_base = disc_cols[1].slider("Base Discount Rate %", 0.0, 50.0, 9.0, 0.1)/100
    disc_bear = disc_cols[2].slider("Bear Discount Rate %", 0.0, 50.0, 10.0, 0.1)/100
    
    # --- Net Cash ---
    net_cash_input = st.number_input(
        "Net Cash (Cash - Debt) USD (override)", min_value=-1e12, value=(cash_guess - debt_guess) or 0.0, step=1_000_000.0
    )
    
    # --- Compute DCF ---
    scenarios = {"Bull": disc_bull, "Base": disc_base, "Bear": disc_bear}
    def compute_scenario(rate):
        res = dcf_from_fcf_list(fcfs, rate, terminal_growth, method=term_value_method)
    ...
    results = {name: compute_scenario(r) for name, r in scenarios.items()}
